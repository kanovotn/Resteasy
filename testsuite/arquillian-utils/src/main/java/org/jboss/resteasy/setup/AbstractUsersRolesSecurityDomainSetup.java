...
public abstract class AbstractUsersRolesSecurityDomainSetup implements ServerSetupTask {
     ...
     * @param path
     */
    public void createPropertiesFiles(String path) {
        this.USERS_FILE = new File(path, USERS_FILENAME);
        this.ROLES_FILE = new File(path, ROLES_FILENAME);
    }
     ...
     */
    public abstract void setConfigurationPath();
    ...
    private void configureElytron() throws Exception {
        ...
        managementClient.executeCli("/subsystem=elytron/properties-realm="
                + ELYTRON_PROPERTIES_REALM_NAME + ":add(users-properties={path=" +
                USERS_FILE.getAbsolutePath() + ",plain-text=true},groups-properties={path=" +
                ROLES_FILE.getAbsolutePath() + "})");
        ...
        assertTrue("The login module should be created", ops.exists(ELYTRON_PROPERTIES_REALM_ADDRESS));
        assertTrue("The login module should be created", ops.exists(ELYTRON_SECURITY_DOMAIN_ADDRESS));
        assertTrue("The login module should be created", ops.exists(ELYTRON_PROP_HTTP_AUTHENTICATION_FACTORY_ADDRESS));
        assertTrue("The login module should be created", ops.exists(UNDERTOW_APPLICATION_SECURITY_DOMAIN_ADDRESS));
    }
